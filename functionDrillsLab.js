...
printName= (someName) => {console.log(someName)}
printName('Cameron')
...
nameCheck = (userName) => {
  return userName === 'Steven' ? 'What is up Steven?'
  : userName === 'Bryan' ? 'Hey Bryan'
  : `Cool name, ${userName}` ;}

  let nameGreeting = console.log(nameCheck('Macey'))
...
faveColorFinder = (color) => {
  ...
  : color === 'you need to evaluate your favorite color choice'
}
...
printAllNames = (anArray) => {for(let i = 0; i < anArray.length; i++){
...
thatsOdd = (varNum) => {
  return Number.isInteger(varNum/2) ? `That's not odd!`
  : 'That is odd indeed!' 
} 

let oddChecker = console.log(thatsOdd(7))
...
faveMovie = console.log(bestMovie('Spirited Away'))
...
theEliminator = (conArray, strLoser) => {
  for (i=0 ; i < conArray.length ; i++){
    if (conArray[i] === strLoser){
      conArray.splice(i,1)
    }
  } 
  return conArray
...
upperLog = (oneString) => {
  oneString = console.log(oneString.toUpperCase())
}
...
emailCheck = (email) => {
   let makeString = String(email)
   let trimString = makeString.trim(' ')
...
console.log(emailCheck('maceytbell@gmail.com '))
////////////////// PROBLEM 19 ////////////////////
/*
...
frogBudget = (gold) => gold / 3
let totalFrogs = console.log(frogBudget(1000))
...
let totalFrogs2 = console.log(frogBudget2(1000))
...
let bathroomScope = ['rubberDuck', 'squeaky']
